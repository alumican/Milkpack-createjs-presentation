class SimpleCreateJS
    ...
    constructor: (canvasId, frameRate = 60, mouseRate = 0, onResize = null) ->
        ...
        @_$canvas.css({ position: 'absolute', 'z-index': 1 })

        # ウィンドウサイズ参照用の div を作成する
        @_$sizeref = $('<div>')
        @_$sizeref.css({ position: 'absolute', 'z-index': 0, width: '100%', height: '100%' })
    ...
    createLoadQueue: (files, plugins = null, onLoad = null) ->
        ...
        # イベントハンドラを登録する
        queue.addEventListener('progress', @_loadProgressHandler)
    ...
    # リソースの読み込み更新時に呼び出されるイベントハンドラ
    _loadProgressHandler: (event) =>
        queue = event.target
