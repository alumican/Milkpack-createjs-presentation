...
class Application extends jpp.milkpack.Milkpack
    ...
    onInit: () ->
        ...
        #----------------------------------------
        # リソースを読み込む
        @preloader = new createjs.Text('0', '140px Arial', '#333')
        @preloader.textAlign = 'center'
        @preloader.regY = 70
        @stage.addChild(@preloader)
        @resize()

        @resourcesById = {}
        files = ({ id: "#{i}", src:"#{@root}image/page_#{('000' + (i + 1)).substr(-4, 4)}.png" } for i in [0...@pageLength])
        loadQueue = simpleCreatejs.createLoadQueue(files, createjs.Sound)
        loadQueue.onProgress = (queue) =>
            ratio = queue.loaded / queue.total
            @preloader.text = Math.floor(ratio * 100)
            
        loadQueueFunc = () => loadQueue.load()

        @addCommand(
            new jpp.command.Func(loadQueueFunc, [], loadQueue, 'complete')
            new jpp.command.Tween(@preloader, { alpha: 0 }, null, 0.5, jpp.util.Easing.easeOutQuart)
            () =>
    ...
    resize: () ->
        ...
        if @preloader
            @preloader.x = @stageWidth  / 2
            @preloader.y = @stageHeight / 2
